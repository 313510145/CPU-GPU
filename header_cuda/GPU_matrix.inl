...
GPU_matrix<T> add_2d(const GPU_matrix<T>& summand, const T& addend) {
    ...
    add_kernel_2d<<<block_num, thread_per_block>>>(summand.data, addend, result.data, summand.rows, summand.cols);
    return result;
}
...
GPU_matrix<T> add_2d(const GPU_matrix<T>& summand, const GPU_matrix<T>& addend) {
    ...
    dim3 thread_per_block(THREAD_NUM_SQUARE_ROOT, THREAD_NUM_SQUARE_ROOT);
    dim3 block_num((summand.rows + thread_per_block.x - 1) / thread_per_block.x, (summand.cols + thread_per_block.y - 1) / thread_per_block.y);
    add_kernel_2d<<<block_num, thread_per_block>>>(summand.data, addend.data, result.data, summand.rows, summand.cols);
    return result;
}
...
GPU_matrix<T> add_async(cudaStream_t& stream, const GPU_matrix<T>& summand, const T& addend) {
    ...
    dim3 thread_per_block(THREAD_NUM);
    dim3 block_num((summand.rows * summand.cols + thread_per_block.x - 1) / thread_per_block.x);
    add_kernel<<<block_num, thread_per_block, 0, stream>>>(summand.data, addend, result.data, summand.rows, summand.cols);
    return result;
}
...
GPU_matrix<T> add_async(cudaStream_t& stream, const GPU_matrix<T>& summand, const GPU_matrix<T>& addend) {
    ...
    dim3 thread_per_block(THREAD_NUM);
    dim3 block_num((summand.rows * summand.cols + thread_per_block.x - 1) / thread_per_block.x);
    add_kernel<<<block_num, thread_per_block, 0, stream>>>(summand.data, addend.data, result.data, summand.rows, summand.cols);
    return result;
}
...
GPU_matrix<T> add_2d_async(cudaStream_t& stream, const GPU_matrix<T>& summand, const T& addend) {
    ...
    dim3 thread_per_block(THREAD_NUM_SQUARE_ROOT, THREAD_NUM_SQUARE_ROOT);
    dim3 block_num((summand.rows + thread_per_block.x - 1) / thread_per_block.x, (summand.cols + thread_per_block.y - 1) / thread_per_block.y);
    add_kernel_2d<<<block_num, thread_per_block, 0, stream>>>(summand.data, addend, result.data, summand.rows, summand.cols);
    return result;
}
...
GPU_matrix<T> add_2d_async(cudaStream_t& stream, const GPU_matrix<T>& summand, const GPU_matrix<T>& addend) {
    ...
    dim3 thread_per_block(THREAD_NUM_SQUARE_ROOT, THREAD_NUM_SQUARE_ROOT);
    dim3 block_num((summand.rows + thread_per_block.x - 1) / thread_per_block.x, (summand.cols + thread_per_block.y - 1) / thread_per_block.y);
    add_kernel_2d<<<block_num, thread_per_block, 0, stream>>>(summand.data, addend.data, result.data, summand.rows, summand.cols);
    return result;
}
...
GPU_matrix<T> subtract(const GPU_matrix<T>& minuend, const T& subtrahend) {
    ...
    subtract_kernel<<<block_num, thread_per_block>>>(minuend.data, subtrahend, result.data, minuend.rows, minuend.cols);
    return result;
}
...
GPU_matrix<T> subtract(const GPU_matrix<T>& minuend, const GPU_matrix<T>& subtrahend) {
    ...
    dim3 thread_per_block(THREAD_NUM);
    dim3 block_num((minuend.rows * minuend.cols + thread_per_block.x - 1) / thread_per_block.x);
    subtract_kernel<<<block_num, thread_per_block>>>(minuend.data, subtrahend.data, result.data, minuend.rows, minuend.cols);
    return result;
}
...
GPU_matrix<T> subtract_2d(const GPU_matrix<T>& minuend, const T& subtrahend) {
    ...
    subtract_kernel_2d<<<block_num, thread_per_block>>>(minuend.data, subtrahend, result.data, minuend.rows, minuend.cols);
    return result;
}
...
GPU_matrix<T> subtract_2d(const GPU_matrix<T>& minuend, const GPU_matrix<T>& subtrahend) {
    ...
    dim3 thread_per_block(THREAD_NUM_SQUARE_ROOT, THREAD_NUM_SQUARE_ROOT);
    dim3 block_num((minuend.rows + thread_per_block.x - 1) / thread_per_block.x, (minuend.cols + thread_per_block.y - 1) / thread_per_block.y);
    subtract_kernel_2d<<<block_num, thread_per_block>>>(minuend.data, subtrahend.data, result.data, minuend.rows, minuend.cols);
    return result;
}
...
GPU_matrix<T> subtract_async(cudaStream_t& stream, const GPU_matrix<T>& minuend, const T& subtrahend) {
    ...
    dim3 thread_per_block(THREAD_NUM);
    dim3 block_num((minuend.rows * minuend.cols + thread_per_block.x - 1) / thread_per_block.x);
    subtract_kernel<<<block_num, thread_per_block, 0, stream>>>(minuend.data, subtrahend, result.data, minuend.rows, minuend.cols);
    return result;
}
...
GPU_matrix<T> subtract_async(cudaStream_t& stream, const GPU_matrix<T>& minuend, const GPU_matrix<T>& subtrahend) {
    ...
    dim3 thread_per_block(THREAD_NUM);
    dim3 block_num((minuend.rows * minuend.cols + thread_per_block.x - 1) / thread_per_block.x);
    subtract_kernel<<<block_num, thread_per_block, 0, stream>>>(minuend.data, subtrahend.data, result.data, minuend.rows, minuend.cols);
    return result;
}
...
GPU_matrix<T> subtract_2d_async(cudaStream_t& stream, const GPU_matrix<T>& minuend, const T& subtrahend) {
    ...
    dim3 thread_per_block(THREAD_NUM_SQUARE_ROOT, THREAD_NUM_SQUARE_ROOT);
    dim3 block_num((minuend.rows + thread_per_block.x - 1) / thread_per_block.x, (minuend.cols + thread_per_block.y - 1) / thread_per_block.y);
    subtract_kernel_2d<<<block_num, thread_per_block, 0, stream>>>(minuend.data, subtrahend, result.data, minuend.rows, minuend.cols);
    return result;
}
...
GPU_matrix<T> subtract_2d_async(cudaStream_t& stream, const GPU_matrix<T>& minuend, const GPU_matrix<T>& subtrahend) {
    ...
    dim3 thread_per_block(THREAD_NUM_SQUARE_ROOT, THREAD_NUM_SQUARE_ROOT);
    dim3 block_num((minuend.rows + thread_per_block.x - 1) / thread_per_block.x, (minuend.cols + thread_per_block.y - 1) / thread_per_block.y);
    subtract_kernel_2d<<<block_num, thread_per_block, 0, stream>>>(minuend.data, subtrahend.data, result.data, minuend.rows, minuend.cols);
    return result;
}
...
GPU_matrix<T> multiply(const GPU_matrix<T>& multiplicand, const T& multiplier) {
    ...
    multiply_kernel<<<block_num, thread_per_block>>>(multiplicand.data, multiplier, result.data, multiplicand.rows, multiplicand.cols);
    return result;
}
...
GPU_matrix<T> multiply_2d(const GPU_matrix<T>& multiplicand, const T& multiplier) {
    ...
    multiply_kernel_2d<<<block_num, thread_per_block>>>(multiplicand.data, multiplier, result.data, multiplicand.rows, multiplicand.cols);
    return result;
}
